...
describe('Edit Settings for Non-ASCII VM', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                          Edit Settings for Non-ASCII VM                                 ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM', 'Edit VM', 'Edit Settings for Non-ASCII VM', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit RAM memory and hard disk size for non-ASCII VM and verify', function () {
        var cancelButton, editSettingsDialog;

        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            return racetrack.log("Change the non-ASCII VM memory to 2048 MB");
        }).then(function () {
            return vmUtil.editVMmemory(VMPage, EsxuiPage, vmName, '2048');
        }).then(function () {
            ...
            // Check whether the values are actually modified
            ...
            var ramMemory = editSettingsDialog.memoryTextbox(0);
            return expect(ramMemory.getAttribute('value')).toEqual('2048');
        }).then(function () {

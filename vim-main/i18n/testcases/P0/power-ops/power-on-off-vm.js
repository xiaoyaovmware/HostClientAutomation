...
describe('Power on/off non-ASCII VM', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                             Power on/off non-ASCII VM                                   ");
        console.log("-----------------------------------------------------------------------------------------");


        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Power on/off non-ASCII VM', 'Power ops', 'Power on/off non-ASCII VM ', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Power_onoff_non-ASCII_VM_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Power_onoff_non-ASCII_VM_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Power on non-ASCII VM and then power off', function () {

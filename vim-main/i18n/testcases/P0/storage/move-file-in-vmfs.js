...
describe('Move non-ASCII file in VMFS', function () {
    ...
    var loginUtil = new LoginUtil(),
        ...
        vmName, nonAsciiFolderName;

    beforeEach(function () {

        console.log("-----------------------------------------------------------------------------------------");
        console.log("                            Move non-ASCII file in VMFS                                  ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Move non-ASCII file in VMFS', 'Storage', 'Move non-ASCII file in VMFS', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Move_non-ASCII_file_in_VMFS_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Move_non-ASCII_file_in_VMFS_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Move non-ASCII file in VMFS', function () {
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Click on the VM folder and move it to the new created non-ASCII folder");
        }).then(function () {
            var folderNeedToMove = StoragePage.datastoresTab.datastoreBrowser.getFolderByName(vmName);
            var destinationFolder = nonAsciiFolderName;
            return storageUtil.moveFolderInDatastore(StoragePage,folderNeedToMove,destinationFolder)
        }).then(function () {
            return racetrack.log("Verify that VM folder is moved to the new non-ASCII folder");
        }).then(function () {
            return esxuiUtil.checkForRecentTask('Move Datastore File', 'None', browser.params.taskMsg.task.state.success,3);
        }).then(function () {

...
describe('Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("            Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent          ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent', 'Edit VM', 'Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Independent_Non_Persistent_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Independent_Non_Persistent_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent', function () {
        var saveButton;
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Change the non-ASCII VM's disk mode to Independent Non Persistent");
        }).then(function () {
            var editSettingsDialog = VMPage.createRegisterVMButton.newVMWizard;
            return vmUtil.customizedDiskMode(editSettingsDialog,0,2);
        }).then(function () {

...
describe('Edit Settings for Non-ASCII VM Add Network Adapter', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                    Edit Settings for Non-ASCII VM Add Network Adapter                   ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM Add Network Adapter', 'Edit VM', 'Edit Settings for Non-ASCII VM Add Network Adapter', browser.params.i18n.lang, '', '', 'UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Add_Network_Adapter_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Add_Network_Adapter_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM Add Network Adapter', function () {
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Add a new Network Adapter.");
        }).then(function () {
            editSettingsDialog = VMPage.createRegisterVMButton.newVMWizard;
            return vmUtil.addNetworkAdapter(editSettingsDialog, 'addNetworkItem');
            // return vmUtil.addDevice(editSettingsDialog, 'addNetworkItem');
        }).then(function () {
            return racetrack.log("Verify that new Network Adapter is added.");
        }).then(function () {
            return expect(editSettingsDialog.networkAdapterSlideDown.self(1).isPresent()).toBe(true);
        }).then(function () {

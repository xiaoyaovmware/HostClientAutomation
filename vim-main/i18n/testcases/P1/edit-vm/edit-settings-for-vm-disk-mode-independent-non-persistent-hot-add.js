...
describe('Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent Hot Add', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("        Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent Hot Add      ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent Hot Add', 'Edit VM', 'Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent Hot Add', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Independent_Non_Persistent_Hot_Add_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Independent_Non_Persistent_Hot_Add_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM Disk Mode Independent Non Persistent  Hot Add', function () {
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Change the non-ASCII VM's disk mode to Independent Non Persistent Hot Add");
        }).then(function () {
            return vmUtil.customizedDiskMode(editSettingsDialog,1,2);
        }).then(function () {

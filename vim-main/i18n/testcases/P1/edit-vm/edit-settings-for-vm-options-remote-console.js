...
describe('Edit Settings for Non-ASCII VM Options Remote Console', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                  Edit Settings for Non-ASCII VM Options Remote Console                  ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM Options Remote Console', 'Edit VM', 'Edit Settings for Non-ASCII VM Options Remote Console', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Options_Remote_Console_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Options_Remote_Console_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM Options Remote Console and verify', function () {
        var saveButton;
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Change VM Options: Remote console");
        }).then(function () {
            var editSettingsDialog = VMPage.createRegisterVMButton.newVMWizard;
            return vmUtil.changeVMOptionsRemoteConsole(editSettingsDialog, true, true, 25);
        }).then(function () {

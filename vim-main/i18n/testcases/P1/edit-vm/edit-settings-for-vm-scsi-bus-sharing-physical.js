...
describe('Edit Settings for Non-ASCII VM SCSI Bus Sharing Physical', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("               Edit Settings for Non-ASCII VM SCSI Bus Sharing Physical                  ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM SCSI Bus Sharing Physical', 'Edit VM', 'Edit Settings for Non-ASCII VM SCSI Bus Sharing Physical', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Sharing_Physical_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Sharing_Physical_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM SCSI Bus Sharing Physical', function () {
        var saveButton;
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Change the non-ASCII VM's SCSI controller to bus sharing physical");
        }).then(function () {
            var editSettingsDialog = VMPage.createRegisterVMButton.newVMWizard;
            return vmUtil.customizedSCSIController(editSettingsDialog,0,0,2);
        }).then(function () {

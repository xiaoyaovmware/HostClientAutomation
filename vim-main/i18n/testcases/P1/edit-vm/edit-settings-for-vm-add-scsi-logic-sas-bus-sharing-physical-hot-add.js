...
describe('Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic SAS and Bus Sharing Physical Hot Add', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log(" Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic SAS and Bus Sharing Physical Hot Add ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic SAS and Bus Sharing Physical Hot Add', 'Edit VM', 'Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic SAS and Bus Sharing Physical Hot Add', browser.params.i18n.lang, '', '', 'UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Add_SCSI_controller_LSI_Logic_SAS_and_Bus_Sharing_Physical_Hot_Add_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Add_SCSI_controller_LSI_Logic_SAS_and_Bus_Sharing_Physical_Hot_Add_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic SAS and Bus Sharing Physical Hot Add', function () {
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Add a new SCSI controller - LSI Logic SAS");
        }).then(function () {
            ...
            return racetrack.log("Change the non-ASCII VM's SCSI controller to bus sharing virtual and LSI Logic SAS");
        }).then(function () {
            return vmUtil.customizedSCSIController(editSettingsDialog,1,0,1);
        }).then(function () {

...
describe('Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic Parallel', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("          Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic Parallel          ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic Parallel', 'Edit VM', 'Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic Parallel', browser.params.i18n.lang, '', '', 'UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_Add_SCSI_controller_LSI_Logic_Parallel_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_Add_SCSI_controller_LSI_Logic_Parallel_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM Add SCSI controller LSI Logic Parallel', function () {
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Add a new SCSI controller - LSI Logic Parallel");
        }).then(function () {
            ...
            return globalUtil.waitForVisibility(editSettingsDialog.scsiControllerSlideDown.scsiControllerDropDown.scsiControllerType(1));
        }).then(function () {
            return editSettingsDialog.scsiControllerSlideDown.scsiControllerDropDown.scsiControllerType(1).click();
        }).then(function () {

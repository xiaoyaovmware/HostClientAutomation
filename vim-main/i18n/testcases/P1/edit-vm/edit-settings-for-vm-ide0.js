...
describe('Edit Settings for Non-ASCII VM IDE 0', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                        Edit Settings for Non-ASCII VM IDE 0                             ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Edit Settings for Non-ASCII VM IDE 0', 'Edit VM', 'Edit Settings for Non-ASCII VM IDE 0', browser.params.i18n.lang, '', '', 'UI','P0','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Edit_Settings_for_Non-ASCII_VM_IDE0_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Edit_Settings_for_Non-ASCII_VM_IDE0_Stop',screenshotSavePath).then(function(){
            done();
        });
    });


    it('Edit Settings for Non-ASCII VM IDE 0 and verify', function () {
        var saveButton;
        ...
        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            ...
            return racetrack.log("Change the non-ASCII VM's Virtual Device Node to IDE controller 0");
        }).then(function () {
            var editSettingsDialog = VMPage.createRegisterVMButton.newVMWizard;
            return vmUtil.customizedVirtualDevice(editSettingsDialog,0,0,0);
        }).then(function () {

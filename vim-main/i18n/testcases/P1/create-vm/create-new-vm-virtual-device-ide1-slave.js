...
describe('Create New VM with virtual device IDE 1 Slave', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                  Create New VM with virtual device IDE 1 Slave                          ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Create New VM with virtual device IDE 1 Slave','Create VM','Create New VM with virtual device IDE 1 Slave',browser.params.i18n.lang,'','','UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Create_New_VM_with_virtual_device_IDE_1_Slave_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Create_New_VM_with_virtual_device_IDE_1_Slave_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Create New VM with virtual device IDE 1 Slave', function () {
       ...
       return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function(){
            ...
            return racetrack.log("Create a new non-ASCII VM with 512 MB RAM and 10 MB and Virtual Device Node IDE controller 1 Slave: " + vmName);
        }).then(function(){
           ...
           var windows = browser.params.vmMsg.vm.wizard.basics.osVersion.windowsGuest;
            return vmUtil.createVM(newVMWizard, EsxuiPage, vmName, windows, 'Windows 10 32', 512, 8, 'Customize Virtual Device Node', 0, 1,1);
        }).then(function () {

...
describe('Create New VM with disk mode Independent Non-persistent', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                Create New VM with disk mode Independent Non-persistent                  ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Create New VM with disk mode Independent Non-persistent','Create VM','Create New VM with disk mode Independent Non-persistent',browser.params.i18n.lang,'','','UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Create_New_VM_with_disk_mode_Independent_Non-persistent_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Create_New_VM_with_disk_mode_Independent_Non-persistent_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Create New VM with disk mode Independent Non-persistent', function () {
       ...
       return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function(){
            ...
            return racetrack.log("Create a new non-ASCII VM with disk mode: Independent Non-persistent: " + vmName);
        }).then(function(){
            ...
            return vmUtil.createVM(newVMWizard, EsxuiPage, vmName, windows, 'Windows 10 32', 512, 8, 'Customize Disk Mode', 0, 2);
        }).then(function () {

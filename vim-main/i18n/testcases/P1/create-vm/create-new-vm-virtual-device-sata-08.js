...
describe('Create New VM with virtual device Sata Controller 0:8', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                Create New VM with virtual device Sata Controller 0:8                    ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Create New VM with virtual device Sata Controller 0:8','Create VM','Create New VM with virtual device Sata Controller 0:8',browser.params.i18n.lang,'','','UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Create_New_VM_with_virtual_device_Sata_Controller_0_8_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Create_New_VM_with_virtual_device_Sata_Controller_0_8_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Create New VM with virtual device Sata Controller 0:8', function () {
       ...
       return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function(){
            ...
            return racetrack.log("Create a new non-ASCII VM with Virtual Device Node Sata controller 0:8: " + vmName);
        }).then(function(){
           ...
           var windows = browser.params.vmMsg.vm.wizard.basics.osVersion.windowsGuest;
            return vmUtil.createVM(newVMWizard, EsxuiPage, vmName, windows, 'Windows 10 32', 512, 8, 'Customize Virtual Device Node', 0, 3, 8);
        }).then(function () {

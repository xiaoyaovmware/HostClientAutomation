...
describe('Create New non-ASCII VM with Adapter SR IOV', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                        Create New non-ASCII VM with Adapter SR IOV                      ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Create New non-ASCII VM with Adapter SR IOV ','Create VM','Create New non-ASCII VM with Adapter SR IOV ',browser.params.i18n.lang,'','','UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Create_New_non-ASCII_VM_with_Adapter_SR_IOV_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Create_New_non-ASCII_VM_with_Adapter_SR_IOV_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Create New non-ASCII VM with Adapter SR IOV', function () {
       ...
       return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function(){
            ...
            return racetrack.log("Create New non-ASCII VM with Adapter SR IOV: " + vmName);
        }).then(function(){
           ...
           return vmUtil.createVM(newVMWizard, EsxuiPage, vmName, windows, 'Windows 10 32', 512, 8, 'Customize Adapter', 0, 1);
        }).then(function () {
            ...
            return esxuiUtil.checkForRecentTask('Create VM', 'vm', browser.params.taskMsg.task.state.success, 2);
        }).then(function () {

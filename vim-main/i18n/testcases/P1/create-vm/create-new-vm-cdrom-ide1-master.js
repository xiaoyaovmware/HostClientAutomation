...
describe('Create New non-ASCII VM with CD ROM IDE 1 Master', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                    Create New non-ASCII VM with CD ROM IDE 1 Master                     ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Create New non-ASCII VM with CD ROM IDE 1 Master','Create VM','Create New non-ASCII VM with CD ROM IDE 1 Master',browser.params.i18n.lang,'','','UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Create_New_non-ASCII_VM_with_CD_ROM_IDE_1_Master_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Create_New_non-ASCII_VM_with_CD_ROM_IDE_1_Master_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Create New non-ASCII VM with CD ROM IDE 1 Master', function () {
       ...
       return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function(){
            ...
            return racetrack.log("Create New non-ASCII VM with CD ROM IDE 1 Master " + vmName);
        }).then(function(){
           ...
           return vmUtil.createVM(newVMWizard, EsxuiPage, vmName, windows, 'Windows 10 32', 512, 8, 'Add CDROM', 0, 1, 0);
        }).then(function () {

...
describe('Create New non-ASCII VM with SCSI Bus Sharing Physical', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("                  Create New non-ASCII VM with SCSI Bus Sharing Physical                 ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Create New non-ASCII VM with SCSI Bus Sharing Physical','Create VM','Create New non-ASCII VM with SCSI Bus Sharing Physical',browser.params.i18n.lang,'','','UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Create_New_non-ASCII_VM_with_SCSI_Bus_Sharing_Physical_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Create_New_non-ASCII_VM_with_SCSI_Bus_Sharing_Physical_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Create New non-ASCII VM with SCSI Bus Sharing Physical', function () {
       ...
       return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function(){
            ...
            return racetrack.log("Create new non-ASCII VM with SCSI Bus Sharing Physical: " + vmName);
        }).then(function(){
           ...
           return vmUtil.createVM(newVMWizard, EsxuiPage, vmName, windows, 'Windows 10 32', 512, 8, 'Customize SCSI Controller', 0, 0, 2);
        }).then(function () {

...
describe('Create New non-ASCII VM with Thick provisioned disk (eagerly zeroed)', function () {
    ...
    beforeEach(function () {
        ...
        console.log("-----------------------------------------------------------------------------------------");
        console.log("           Create New non-ASCII VM with Thick provisioned disk (eagerly zeroed)          ");
        console.log("-----------------------------------------------------------------------------------------");

        browser.driver.manage().window().maximize();

        return racetrack.testCaseBegin('Create New non-ASCII VM with Thick provisioned disk (eagerly zeroed)','Create VM','Create New non-ASCII VM with Thick provisioned disk (eagerly zeroed)',browser.params.i18n.lang,'','','UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Create_New_non-ASCII_VM_with_Thick_provisioned_disk_eagerly_zeroed_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult('Create_New_non-ASCII_VM_with_Thick_provisioned_disk_eagerly_zeroed_Stop',screenshotSavePath).then(function(){
            done();
        });
    });

    it('Create New non-ASCII VM with Thick provisioned disk (eagerly zeroed)', function () {
       ...
       return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function(){
            ...
            return racetrack.log("Create a new non-ASCII VM with thick provisioned hard disk (eagerly zeroed): " + vmName);
        }).then(function(){
           ...
           return vmUtil.createVM(newVMWizard, EsxuiPage, vmName, windows, 'Windows 10 32', 512, 8, 'Customize Provisioning', 0, 'eager');
        }).then(function () {

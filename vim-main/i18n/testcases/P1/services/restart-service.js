...
describe('Restart service', function () {
    ...
    beforeEach(function () {
        ...
        return racetrack.testCaseBegin('Restart service', 'Services', 'Restart service', browser.params.i18n.lang, '', '', 'UI','P1','Automation').then(function(){
            return globalUtil.takeScreenshot(screenshotSavePath, 'Restart_service_Start');
        }).then(function() {
    ...
    afterEach(function (done) {
        return globalUtil.verifyResult( 'Restart_service_Stop', screenshotSavePath).then(function(){
            done();
        });
    });

    it('Restart service', function () {

        return racetrack.log('---------------------------------------Start Test Case--------------------------------------').then(function() {
            return racetrack.log("Restart service TSM-SSH in non-ASCII environment");
        }).then(function () {
            return servicesUtil.restartService(ServicesPage, EsxuiPage, 'lbtd'); //modify
        }).then(function () {
            return racetrack.log("Check for recent task and verify that the service is restarted successfully");
        }).then(function () {
            return esxuiUtil.checkForRecentTask('Restart Service', hostName, browser.params.taskMsg.task.state.success,3);
        }).then(function () {
